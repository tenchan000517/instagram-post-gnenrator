...
### **核心理念**
各コンポーネントが独立性を保ちながら、IDベース連携により完璧に協調動作するシステム。投稿タイプID + ペルソナIDが全システム共通言語となり、それぞれ別々に作業をしても最適な結果が得られる設計。
...
2. テーマ選択  
   システム: 学習型の成功テーマリストを表示
   ユーザー: "ES書き方" を選択
   
3. ペルソナ選択
   システム: "ES書き方"の推奨ペルソナを表示（1:N連携）
   - 推奨: 方法論重視型就活生
   - 選択可能: スキル習得型就活生、問題解決型就活生
   ユーザー: 推奨ペルソナを選択
...
5. 自動生成実行
   システム: TypeID + PersonaID + Title でコンテンツ生成
```
...
### **IDベース連携システム**
```
投稿タイプID (TypeID) + ペルソナID (PersonaID) = システム共通言語

例: TypeID=002(学習型) + PersonaID=015(方法論重視型)
→ 全コンポーネントがこのIDペアで最適化動作
```
...
```
【役割】
整理済みデータ + TypeID + PersonaID → 有益性抽出 + テンプレート適合コンテンツ生成
...
【処理】
- ペルソナに対する「有益性」の抽出・識別
- 投稿タイプ×ペルソナの成功パターンに沿った表現
- 確定テンプレートの項目仕様に完全適合した出力
