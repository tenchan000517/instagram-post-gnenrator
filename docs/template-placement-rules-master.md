...
- **具体例**: 12項目のNG習慣→全12項目を`dual_section`で2つずつ詳細表示、8つのクッション言葉→全パターンを`image_point`で記憶アンカー付き表示
- **品質基準**: テンプレートの制限に情報を合わせるのではなく、情報量に適合するテンプレートを選択

#### 🎯 テンプレート組み合わせ戦略
- **基本構造**: pageStructurePatternで基本テンプレート設定（`unified-template-XX-`形式）
...
- **段階的説明**: `dual_section`で準備→実践→応用の段階的プロセスを詳細解説
- **templateOverrides利用可能テンプレート**: `simple_intro`, `dual_section`, `ranking_display`, `item_grid`, `comparison`, `company_detail`, `item_list`, `section_blocks`, `dynamic_boxes`, `image_point`
...
## 📚 参考資料: Type002詳細ルール（作業中）

### Type002 (実践習得型) - 特別ルール
- **基本コンセプト**: 読者が正確な手順で実践できるよう、情報の順序性と完全性を最重視
- **⚠️ 重要**: 手順の正確性が読者の実践成果に直結するため、情報欠損は絶対禁止

#### Type002専用ルール（2025-07-30追加）
...
2. **🎯 テンプレート組み合わせ戦略**:
   ...
   - **手順対比**: templateOverridesで`comparison`を使用し、NG/GOODの対比で正しい手順を明示
   - **段階的説明**: `dual_section`で準備→実践→応用の段階的プロセスを詳細解説
   - **利用可能テンプレート**: `simple_intro`, `dual_section`, `ranking_display`, `item_grid`, `comparison`, `company_detail`, `item_list`, `section_blocks`, `dynamic_boxes`, `image_point`
