...
### **5. 取得コスト（acquisitionCost）**

#### **スコア定義（1-5の整数・低いほど良い）**
```
スコア1：極低コスト（1万円以下＋1ヶ月以内）
スコア2：低コスト（5万円以下＋3ヶ月以内）
スコア3：中コスト（20万円以下＋6ヶ月以内）
スコア4：高コスト（100万円以下＋2年以内）
スコア5：極高コスト（100万円超または2年超）
```
...
```javascript
// 総合スコアの計算
totalScore = difficultyLevel + specialization + careerValue + incomeImpact + (6 - acquisitionCost)

// 注意：取得コストは逆転評価（低いほど良い）
// 最小値：5、最大値：25
```
...
function validateQualificationData(data) {
  const rules = {
    ...
    acquisitionCost: { type: 'integer', min: 1, max: 5 }
  };
