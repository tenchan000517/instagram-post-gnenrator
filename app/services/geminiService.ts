...
- マークダウン記法（**太字**、*斜体*、# 見出し等）は一切使用しない
- ですます調を基本とし、感嘆符（！）の多用は避ける
- カジュアルな親しみやすさを保ちつつ、フランクすぎない適度な距離感を維持
- 個性的な表現や独特の言い回しは避け、万人に受け入れられる標準的な文体を使用
- 文末は「です」「ます」「ください」等で統一し、自然な丁寧語を心がける
...
`
  ...
  private getFallbackAnalysis(content: string, strategy: StrategyType): AIAnalysisResult {
    ...
    return {
      ...
      caption: `学生の皆さんの成長を応援しています。\n\n${content.substring(0, 100)}...\n\nFIND to DO で一緒に成長していきませんか。詳しくはプロフィールをご覧ください。`
    }
  }
