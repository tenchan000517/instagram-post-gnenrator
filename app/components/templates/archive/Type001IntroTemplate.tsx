...
interface Type001IntroTemplateProps {
  data: {
    title?: string;
...
export default function Type001IntroTemplate({ data }: Type001IntroTemplateProps) {
  const {
    title = '',
    targetAudience = '',
    problems = [],
...
export const type001IntroMetadata: TemplateMetadata = {
  id: 'type001_intro',
  name: 'Type001導入型',
  description: 'Type001用の導入テンプレート。タイトル、ターゲット質問、問題リストを表示。misaki_worry.png使用、ピンク背景',
  suitableFor: {
    contentTypes: ['導入', '問題提起', 'イントロダクション'],
    genres: ['キャリアの悩み解決', '感情共感', 'Type001'],
  ...
  keywords: ['導入', '問題提起', 'Type001', '感情共感', 'キャリア']
}