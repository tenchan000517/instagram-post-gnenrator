...
### システムフロー理解について（2025-07-17追加）

**重要**: ジャンル判定からテンプレート表示までの完全なシステムフローを理解するため、以下のドキュメントを必ず参照してください：

**参照ドキュメント**: 
- `docs/system-flow-analysis.md` - システム全体構造（理解用）
- `docs/complete-system-flow.md` - **完全フロー解明書（作業用・必読）**

**🚨 重要**: `complete-system-flow.md`は「蝶の羽ばたき効果」を考慮した完全版です。
- **フォーマッターから最終表示まで**: 5回のAI呼び出しを含む完全フロー
- **修正影響範囲**: 一行の修正がシステム全体に波及することの理解
- **実際のデータフロー**: URLパラメータ、LocalStorage連携の詳細
- **脆弱性の発見**: システムの弱点と修正時の注意点

**修正作業前に必ずcomplete-system-flow.mdを熟読してください。**
