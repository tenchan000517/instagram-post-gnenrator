...
3. /mnt/c/instagram-course/instagram-post-generator/app/data/companyDatabase/rankings/femaleCareer/配下から該当ファイル特定
4. ファイル存在確認・読み込み
```
...
```
パターン1：共感寄り添い型
「同じように感じてる女性の方、きっと多いと思います。一緒に解決していきましょうね♪」
...
✅ **正確・誠実（必須使用）**
- **source**: 「FIND to DO調べ・MISAKIデータベース」
- **note**: 「FIND to DOコミュニティ独自調査による」
- **subtitle**: 「FIND to DOコミュニティ調べ」

### **タイトル作成の3原則**
1. **女性に寄り添う優しい表現**を使う（MISAKIらしさ）
2. **「TOP10」は冗長なので削除**する
